...
### Tmux
Terminal multiplexer with Tokyo Night theme and vim-tmux-navigator. Session ordering is stable across reboots — pinned sessions (per the machine-local file `~/.config/tmux/session-order`, one name per line) appear first, then alphabetical. The repo ships only the script (`sessions-ordered`) and a template (`session-order.example`); the actual pin list is intentionally machine-local. `prefix f` opens an indexed display-menu (press digit 1-9 or 0 to jump); `prefix F` opens an fzf fuzzy picker; `prefix s 1-9,0` jumps directly without a menu. All three use the same ordered list. Also: `prefix l` for last-session toggle. Directional pane swapping with `prefix H/J/K/L` (complements `Ctrl+hjkl` navigation). Session persistence (resurrect/continuum) disabled due to instability. Window-status pills use rounded powerline caps (U+E0B6/E0B4) with a `󰔱` zoom indicator on the active window, plus a `#{@hugr_state}` slot per window for Hugr's live agent-state glyph (▶ working / ⏸ blocking / ✦ done — pushed by the hugr notifier on `.agent-state` transitions).
